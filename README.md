# BASS: Budget-Aware Subgraph Selection for Long-Video Query Processing
...
**BASS** is a **training-free** framework for long-video query processing. It reformulates video token selection as a **constrained subgraph selection problem**, achieving SOTA performance on VideoMME, VRBench, and CinePile with minimal token usage.
...
As large multimodal models (LMMs) become the dominant approach for video analysis, managing the massive volume of visual tokens in long-form videos has become a critical data management challenge. Naively feeding full-length video streams into LMMs results in excessive token usage. To overcome this, we propose BASS (Budget-Aware Subgraph Selection), a training-free framework that reformulates long-video token selection as a subgraph selection problem under a fixed token budget.
Our method first builds an offline semantic index by segmenting the video into event nodes and constructing a directed weighted graph that captures both sequential flow and long-range structural dependencies. During online inference, BASS utilizes a monotone submodular objective function to select the most informative subgraph within a strict token budget, with provable approximation guarantees. In addition, we design a graph-guided execution pipeline that treats the selected subgraph as a structured query plan, forcing the LMM to verify visual evidence and perform logical propagation along validated graph paths to mitigate hallucinations. Extensive experiments across multiple long-video benchmarks demonstrate that BASS achieves state-of-the-art performance.
...
> **Figure 1**: The overall framework of BASS. We first decompose the video into events, construct a weighted graph capturing semantic and temporal links, and then select a budget-constrained subgraph to guide the LMM's reasoning.

<div align="center">
  <img src="assets/framework.png" width="95%" alt="BASS Framework"/>
  <br>
</div>
...
```bibtex
@article{bass2026,
  title={BASS: Budget-Aware Subgraph Selection for Long-Video Query Processing},
